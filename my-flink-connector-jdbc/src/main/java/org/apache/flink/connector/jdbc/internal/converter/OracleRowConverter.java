	...
	protected JdbcDeserializationConverter createInternalConverter(LogicalType type) {
		switch (type.getTypeRoot()) {
			...
			case DATE:
				return val -> {
					if (val instanceof TIMESTAMP) {
						return (int) (((TIMESTAMP) val).dateValue().toLocalDate().toEpochDay());
					} else if (val instanceof Timestamp) {
						return (int) (((Timestamp) val).toLocalDateTime().toLocalDate().toEpochDay());
					} else if (val instanceof DATE) {
						return (int) (((DATE) val).dateValue().toLocalDate().toEpochDay());
					}
			...
			case TIME_WITHOUT_TIME_ZONE:
				return val -> {
					if (val instanceof TIMESTAMP) {
						return (int) (((TIMESTAMP) val).timeValue().toLocalTime().toNanoOfDay()
							/ 1_000_000L);
					} else if (val instanceof Timestamp) {
			...
			case TIMESTAMP_WITHOUT_TIME_ZONE:
				return val -> {
					if (val instanceof TIMESTAMP) {
						...
						return TimestampData.fromTimestamp(((DATE) val).timestampValue());
					} else if (val instanceof Date) {
